<?php
  ...
  if(isset($_SESSION['logged_in'])){
    if(isset($_GET['id'])){
      ...
      $result = $pdo->query("SELECT * FROM ccw_courses WHERE ccw_id = '$id'");

      $row_array = $result->fetch(PDO::FETCH_ASSOC);
      $ccw_title = $row_array['ccw_title'];
      $ccw_tag = $row_array['ccw_tag'];
      $ccw_info = $row_array['ccw_info'];
      $ccw_learn = $row_array['ccw_learn'];
      $ccw_equipment = $row_array['ccw_equipment'];
    }
...
<head>
  ...
  <title>Admin | Edit C.C.W. Course</title>
  ...
  <body>
    ...
    <header id="header">
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-12">
            <h1><span><img src="../../images/admin-header.png" height="100px"></span> Admin Access <small> C.C.W. Courses</small></h1>
          </div>
        </div>
      </div>
    </header>
    ...
    <section id="breadcrumb">
      <div class="container-fluid">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="ccw.php">C.C.W.</a></li>
    ...
    <section class="main">
      <div class="container-fluid add-course">
        <span class="float-left"><a class="btn btn-secondary" href="ccw.php">&larr; Cancel</a></span>
        <h2 style="text-align: center;">Edit <?php echo $ccw_tag; ?></h2>
        ...
        <form class="add-course" action="update.php" method="post" autocomplete="off">
          ...
          <textarea name="title" rows="1" cols="57"><?php echo $ccw_title; ?></textarea>
          <br>
          <br>
          <label for="tag"><h5><strong>Course Tag</strong></h5></label>
          <input type="text" name="tag" value="<?php echo $ccw_tag; ?>">
          <br>
          <br>
          <label for="prerequisites"><h5><strong>Prerequisites</strong></h5></label>
          <textarea name="prerequisites" rows="4" cols="60"><?php echo $ccw_prerequisites; ?></textarea>
          <br>
          <br>
          <label for="title"><h5><strong>Course Info</strong></h5></label>
          <textarea name="info" rows="6" cols="70"><?php echo nl2br($ccw_info); ?></textarea>
          <br>
          <br>
          <div class="row">
            <div class="col-md-6">
              <label for="learn"><h5><strong>What will be Learned</strong></h5></label>
              <textarea name="learn" rows="12" cols="70"><?php echo $ccw_learn; ?></textarea>
            </div>
            <div class="col-md-6">
              <label for="equipment"><h5><strong>Equipment Required</strong></h5></label>
              <textarea name="equipment" rows="12" cols="70"><?php echo $ccw_equipment; ?></textarea>
            </div>
          </div>
          <br>
